...
static GLuint createGLTextureFromImage(const unsigned char* data, int w, int h, int comp, bool srgb = false)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);   // 필요 시 mipmap 켜기
...
void load_mtl(const std::string& filePath) {
    ...
    std::string line;
    Material current;            // ← header.h의 Material 사용
    ...
    while (std::getline(fin, line)) {
        ...
        if (tag == "newmtl") {
            if (hasCurrent) gMaterials.push_back(current);   // ← header.h의 gMaterials 사용
            current = Material();
            iss >> current.name;
            hasCurrent = true;
        }
